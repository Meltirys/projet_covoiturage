                ...
                <div class="mb-6">
                    ...
                    <ul class="flex flex-col gap-2">
                        ...
                        <?php if (empty($driveUpcoming)): ?>
                            <p class="text-sm text-[#9AA5B4]">Aucun trajet à venir.</p>
                        <?php else: ?>
                            <?php foreach ($driveUpcoming as $journey): ?>
                                <li class="flex justify-between items-center bg-white border border-[rgba(37,63,114,0.25)] rounded-xl px-4 py-3">
                                    <div>
                                        ...
                                        ?>
                                        <p class="text-sm font-poppins text-[#253F72]"><?= format_date_fr($journey['departure']) ?></p>
                                    </div>
                                    <span class="text-xs font-poppins text-[#253F72] bg-[#D6E0F0] rounded-full px-3 py-0.5"><?= esc($journey['places_restantes']) ?>/<?= esc($journey['number_of_place']) ?></span>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                        <!-- boucle trajets passés -->
                        <?php if (empty($drivePast)): ?>
                            <p class="text-sm text-[#9AA5B4]">Aucun trajet passé.</p>
                        <?php else: ?>
                            <?php foreach ($drivePast as $journey): ?>
                                <li class="flex justify-between items-center bg-white border border-[rgba(37,63,114,0.25)] rounded-xl px-4 py-3">
                                    ...
                                    <span class="text-xs font-poppins text-[#9AA5B4] bg-[#D6E0F0] rounded-full px-3 py-0.5">Effectué</span>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </ul>
                    ...
                    <div class="mb-6">
                        ...
                        <ul class="flex flex-col gap-2">
                            ...
                            <?php if (empty($pendingRequests)): ?>
                                <p class="text-sm text-[#9AA5B4]">Aucune demande en attente.</p>
                            <?php else: ?>
                                <?php foreach ($pendingRequests as $request): ?>
                                    <li class="flex justify-between items-center bg-white border border-[rgba(37,63,114,0.25)] rounded-xl px-4 py-3">
                                        <div>
                                            <?php // Villes — à décommenter quand Track sera défini 
                                            ?>
                                            <p class="text-sm font-poppins text-[#253F72]"><?= esc($request['passenger_name']) ?></p>
                                            <p class="text-xs text-[#9AA5B4]"><?= esc($request['journey']['departure']) ?></p>
                                        </div>
                                        <div class="flex gap-2">
                                            <form action="<?= site_url('reservation/accepter/' . $request['id_booking']) ?>" method="post">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="text-xs text-[#253F72] border border-[rgba(37,63,114,0.25)] rounded-full px-3 py-1 hover:bg-[#D6E0F0] transition-colors duration-150">✓</button>
                                            </form>
                                            <form action="<?= site_url('reservation/refuser/' . $request['id_booking']) ?>" method="post">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="text-xs text-[#E05353] border border-[rgba(224,83,83,0.3)] rounded-full px-3 py-1 hover:bg-red-50 transition-colors duration-150">✗</button>
                                            </form>
                                        </div>
                                    </li>
                                <?php endforeach; ?>
                ...
                <div class="mb-6">
                    ...
                    <ul class="flex flex-col gap-2 mb-3">
                        <?php if (empty($upcomingConfirmed)): ?>
                            <p class="text-sm text-[#9AA5B4]">Aucun trajet confirmé.</p>
                        <?php else: ?>
                            <?php foreach ($upcomingConfirmed as $booking): ?>
                                <li class="flex justify-between items-center bg-white border border-[rgba(37,63,114,0.25)] rounded-xl px-4 py-3">
                                    <div>
                                        <?php // Villes — à décommenter quand Track sera défini 
                                        ?>
                                        <p class="text-sm font-poppins text-[#253F72]"><?= format_date_fr($booking['journey']['departure']) ?></p>
                                        <p class="text-xs text-[#9AA5B4]"><?= esc($booking['driver_name']) ?></p>
                                    </div>
                                    ...
                                    <form action="<?= site_url('reservation/annuler/' . $booking['id_booking']) ?>" method="post">
                                        <?= csrf_field() ?>
                                        <button type="submit" class="text-xs text-[#E05353] border border-[rgba(224,83,83,0.3)] rounded-full px-3 py-1 hover:bg-red-50 transition-colors duration-150">Annuler</button>
                                    </form>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <h4 class="text-xs font-poppins text-[#9AA5B4] mb-2">En attente</h4>
                    <ul class="flex flex-col gap-2 mb-3">
                        ...
                        <?php else: ?>
                            <?php foreach ($pastJourney as $booking): ?>
                                <li class="flex justify-between items-center bg-white border border-[rgba(37,63,114,0.25)] rounded-xl px-4 py-3">
                                    ...
                                    <span class="text-xs font-poppins text-[#9AA5B4] bg-[#D6E0F0] rounded-full px-3 py-0.5"><?= $booking['is_validated'] ? 'Effectué' : 'Refusé' ?></span>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </ul>
